...
class DriftManager:
    ...
    def __init__(self, retrain_threshold: float = 0.5, reduce_threshold: float = 0.2, min_multiplier: float = 0.5):
    ...
    def evaluate(self, statistics: Iterable[float]) -> DriftAction:
        ...
        if max_stat >= self.retrain_threshold:
            return DriftAction(retrain=True, sizing_multiplier=self.min_multiplier, message="Drift exceeds retrain threshold")
        if max_stat >= self.reduce_threshold:
            return DriftAction(retrain=False, sizing_multiplier=self.min_multiplier, message="Drift high: reduce sizing")
